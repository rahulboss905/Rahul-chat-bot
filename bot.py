...
def init_db():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS welcome (
        ...
        message TEXT)""")
    cur.execute("INSERT OR IGNORE INTO welcome (id, message) VALUES (1, '👋 Hi! Welcome to the bot.')")
    conn.commit()
    conn.close()
...
/stats – Show user and group count
...
async def broadcast_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for cid in ids:
        try:
            ...
            continue
    await update.message.reply_text(f"✅ Broadcast done: {success} sent.")
    return -1
...
async def forward_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    formatted = f"✉️ Message from {chat.first_name} ({chat.id}):\n{text}"
...
async def reply_to_owner_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message and update.message.reply_to_message:
        ...
        if orig.startswith("✉️ Message from") and "(" in orig:
            try:
...
def main():
    ...
    app.add_handler(MessageHandler(filters.REPLY & filters.TEXT, reply_to_owner_handler))
